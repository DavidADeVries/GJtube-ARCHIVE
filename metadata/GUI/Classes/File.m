classdef File
    ...
    properties
        ...
        tuningPoints = TuningPoint.empty; %are points that can be dragged to correct the segmentation and then become new waypoints
        tubePoints = [];
        refPoints = [];
        midlinePoints = [];
    ...
    methods
        ...
        %% getNewWaypoints %%
        function [ newWaypointStructs ] = getNewWaypoints(obj)
            %by looking at shifts in the tuning points, a new set of
            %waypoints is created
            
            tuningPoints = obj.tuningPoints;
            numTuningPoints = length(tuningPoints);
            oldWaypoints = obj.getWaypointPoints;
            
            newWaypoints = [];
            
            newWaypoints(1:2,:) = oldWaypoints(1:2,:);
            ...
            while oldWaypointNum <= length(oldWaypoints)
                if tuningPointNum > numTuningPoints
                    newWaypoints(newWaypointNum,:) = oldWaypoints(oldWaypointNum,:);
                ...
                elseif tuningPoints{tuningPointNum}.spaceNumber > spaceNum
                    ...
                    if obj.roiOn
                        currentTuningPointPos = roiToNonRoi(obj.roiCoords, currentTuningPointPos);
                    end
                    
                    
                    if isequal(currentTuningPointPos, tuningPoints{tuningPointNum}.originalPosition) %point has moved, so not being used
            ...
            numNewWaypoints = length(newWaypoints);
            
            newWaypointStructs = Waypoint.empty(numNewWaypoints,0);
            
            for i=1:numNewWaypoints
                roiOn = false; %already have performed the necessary transform
                roiCoords = []; %don't really need them
                
                newWaypointStructs(i) = Waypoint(newWaypoints(i,:), roiOn, roiCoords);
            end
        end
